...
import { Card, CardBody, CardTitle, ListGroup, ListGroupItem, Input } from 'reactstrap';
    ...
    <Card>
      ...
      <ListGroup flush>
        {earningData.map((earning) => (
          ...
          >
            <Image src={earning.img} alt={earning.img} className="rounded-circle" width="45" height="45" />
            <h5 className="ms-3 mb-0">{earning.title}</h5>
            <div className="ms-auto">
              <span className={`badge text-dark-white bg-${earning.bg}`}>${earning.earn}</span>
            </div>
          </ListGroupItem>
        ))}
      </ListGroup>
